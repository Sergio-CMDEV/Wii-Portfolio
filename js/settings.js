    ...
    document.querySelectorAll('#setting-page .set-slider input').forEach(elmnt => {
        ...
        elmnt.addEventListener('input', (event) => {
            ...
            if (elmnt.getAttribute('saveto') !== "undefined") {
                var key = elmnt.getAttribute('saveto');
                if (key in userConfig) {
                    userConfig[key] = parseFloat(elmnt.value);
                    // Save to local storage
                    localStorage.setItem('sergiocm-portfolio-settings', JSON.stringify(userConfig));
                    // Apply music volume changes live.
                    if (key === 'musicVol' && typeof bgMusic !== 'undefined' && bgMusic.volume) {
                        bgMusic.volume(userConfig.musicVol);
                    }
                } else {
...
function getSettingHtml(target) {
    ...
    switch (target) {
        case "index":
            return `
            ${makeSettingPage([
                {title: "Efectos de sonido", type: "slider", min: 0, max: 1, step: 0.05, value: userConfig.sfxVol, saveto: "sfxVol"},
                {title: "Música", type: "slider", min: 0, max: 1, step: 0.05, value: userConfig.musicVol, saveto: "musicVol"},
